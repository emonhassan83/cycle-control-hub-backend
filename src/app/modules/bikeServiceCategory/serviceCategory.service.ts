...
const createBikeServiceCategoryIntoDB = async (payload: TBikeServiceCategory) => {
  ...
  if (existServices) {
    throw new AppError(httpStatus.CONFLICT, `${serviceName} service is already exists`);
  }
...
const updateBikeServiceCategoryFromDB = async (categoryId: string, payload: any) => {
  const serviceCategory = await ServiceCategory.findById(categoryId);
  if (!serviceCategory) {
    throw new AppError(httpStatus.NOT_FOUND, "Service category not found!");
  }

  const updateService = await ServiceCategory.findByIdAndUpdate(categoryId, payload, { new: true });
  if (!updateService) {
    throw new AppError(httpStatus.NOT_FOUND, "Service not found and failed to update!");
  }
...
const deleteABikeServiceCategoryFromDB = async (categoryId: string) => {
  ...
  if (!deleteService) {
    throw new AppError(httpStatus.NOT_FOUND, "Service not found and failed to delete!");
  }
